...
| C3 gesture capture | 🟡 | `packages/shell/src/gestures.ts` (selection, text commit, drag/drop through the grid solver, block insert as IR fragments, resize) and `projection/tripwire.ts`. Pure and fully tested. **Not done: the live spike against a running GrapesJS canvas** — `projection/canvas.ts` wires the sorter and selection events to these functions, but the gate ("one full editing session with zero tripwire hits") needs the shell app booted in a browser, which is the next piece of work. |
...
| C6 incremental projection | 🟡 | `ProjectionCanvas.patch()` patches by node id, and ops report exactly which nodes they touched. The 2,000-node benchmark and the p95 < 16ms measurement are not done. |
...
| Stage C3 graft-vs-greenfield | **Not yet callable.** The tripwire, the gesture layer and the projection all exist; the session against a live canvas has not been run. |
