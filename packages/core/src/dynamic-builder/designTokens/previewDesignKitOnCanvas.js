...
const previewDesignKitOnCanvas = (editor, moduleOptions, kitRecord) => {
  ...
  const previewTokens = resolveKitPreviewTokens(
    resolveActiveDesignTokens(editor, moduleOptions),
    moduleOptions,
    kitRecord,
  );
  injectStylesOnce(
    canvasDocument,
    'db-css-designkit-preview-fonts',
    buildDesignKitFontImportCss(kitRecord.fontFamilies),
  );
