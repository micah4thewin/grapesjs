...
| D5 live performance meter | 🟡 | The harness's status bar shows the route's shipped bytes from the same fast-profile compile the canvas is showing, and the spike asserts the meter reads what the gate reads. Budget headroom and click-to-select-the-offender are not wired; the compiler exposes `budget::headroom` for them. |
...
| E3 real data on canvas | 🟡 | The projector takes the record snapshot and renders real rows (the blog corpus site projects real posts, and the spike harness shows them on the canvas). Editing a bound value back to a record needs E2. |
